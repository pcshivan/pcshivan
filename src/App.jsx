...
// SocialIcon component for consistent social media links
const SocialIcon = ({ href, iconClass, label }) => (
  <a
    href={href}
    target="_blank"
    rel="noopener noreferrer"
    className="text-gray-300 hover:text-purple-400 transition-colors duration-300 mx-3 text-2xl transform hover:scale-110"
    aria-label={label}
  >
...
// SectionWrapper for consistent section styling with layered background
const SectionWrapper = ({ id, title, children }) => (
  <section id={id} className="relative py-20 px-4 md:px-8 lg:px-16 bg-gradient-to-br from-gray-900 to-black text-white min-h-screen flex flex-col justify-center items-center overflow-hidden">
    {/* Subtle background pattern/texture for layering */}
    ...
    <h2 className="relative z-10 text-5xl md:text-6xl font-extrabold mb-12 text-center text-purple-400 drop-shadow-lg font-cinzel tracking-wide">
      {title}
    </h2>
...
const Modal = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-xl p-8 max-w-lg w-full relative shadow-2xl border border-purple-600 animate-fade-in-up">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-white text-3xl font-bold transition-colors duration-300"
...
const MessageBox = ({ message, type, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[999]">
      <div className={`${bgColor} rounded-xl p-6 max-w-sm w-full relative shadow-2xl text-white text-center animate-fade-in-up`}>
        <h3 className="text-2xl font-bold mb-4">{title}</h3>
        <p className="mb-6">{message}</p>
        <button
          onClick={onClose}
          className="bg-white text-gray-800 font-bold py-2 px-6 rounded-lg hover:bg-gray-200 transition-colors duration-300 transform hover:scale-105"
        >
          OK
        </button>
      </div>
...
const HomePage = ({ scrollToSection }) => (
  <section id="home" className="relative h-screen flex items-center justify-center bg-cover bg-center overflow-hidden" style={{ backgroundImage: "url('https://placehold.co/1920x1080/1a1a2e/e0b0ff?text=PC+SHIVAN+MUSIC')" }}>
    {/* Dynamic background overlay for artistic effect */}
    ...
    <div className="relative z-10 text-center text-white p-8 rounded-xl bg-gradient-to-br from-gray-900/80 to-black/80 shadow-2xl border border-purple-700 max-w-4xl mx-4 transform animate-fade-in-up">
      ...
      <button
        ...
        className="bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold py-4 px-10 rounded-full text-xl transition-all duration-300 ease-in-out transform hover:scale-105 shadow-lg hover:shadow-xl border border-purple-400"
      >
        Explore My Music
      </button>
    </div>
...
const AboutPage = () => (
  <SectionWrapper id="about" title="About PC Shivan">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
      ...
      <div className="text-lg leading-relaxed text-gray-300 animate-fade-in-right">
        <p className="mb-6">
    ...
    <SectionWrapper id="music" title="My Music">
      <div className="text-center mb-12 animate-fade-in-up">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-block bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white font-bold py-3 px-8 rounded-full text-lg transition-all duration-300 ease-in-out transform hover:scale-105 shadow-lg border border-green-300"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {musicTracks.map((track, index) => (
          <div key={track.id} className="bg-gray-800 rounded-xl shadow-xl overflow-hidden transform transition-all duration-300 hover:scale-105 border border-purple-700 group animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
            <img
              src={track.imageUrl}
              alt={track.title}
            ...
            <div className="p-6">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block w-full text-center bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold py-3 px-6 rounded-lg transition-colors duration-300 transform hover:scale-105 shadow-md"
              >
                Buy Now
              </a>
            </div>
...
const MerchPage = () => {
  ...
  return (
    <SectionWrapper id="merch" title="Merchandise">
      <p className="text-xl text-gray-300 text-center mb-12 animate-fade-in-up">
        Show your support and grab some exclusive PC Shivan merchandise!
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {merchItems.map((item, index) => (
          <div key={item.id} className="bg-gray-800 rounded-xl shadow-xl overflow-hidden transform transition-all duration-300 hover:scale-105 border border-purple-700 group animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
            <img
              src={item.imageUrl}
              alt={item.name}
              className="w-full h-64 object-cover transition-transform duration-300 group-hover:scale-110"
            ...
            <div className="p-6 text-center">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block w-full bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold py-3 px-6 rounded-lg transition-colors duration-300 transform hover:scale-105 shadow-md"
              >
                Buy Now
              </a>
            </div>
...
const TestimonialsPage = ({ userId, db }) => {
  ...
  return (
    <SectionWrapper id="testimonials" title="Fan Love & Testimonials">
      ...
      <form onSubmit={handleSubmitTestimonial} className="bg-gray-800 p-8 rounded-xl shadow-xl mb-12 border border-purple-700 max-w-2xl mx-auto animate-fade-in-up">
        ...
        <button
          type="submit"
          className="w-full bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold py-4 px-6 rounded-lg text-xl transition-colors duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl border border-purple-400"
        >
          Submit Testimonial
        </button>
      </form>
      ...
      {testimonials.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div
              key={testimonial.id}
              className={`p-6 rounded-xl shadow-xl transform ${getRandomRotation()} ${getRandomBgColor()} text-white border border-gray-700 animate-fade-in`}
              style={{ minHeight: '200px', animationDelay: `${index * 0.1}s` }}
            >
    ...
    <div className="min-h-screen bg-gray-950 font-inter">
      ...
      <Modal isOpen={showContactModal} onClose={closeContactModal}>
        ...
        <form onSubmit={handleContactSubmit}>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold py-3 px-6 rounded-lg text-xl transition-colors duration-300 transform hover:scale-105 shadow-lg"
          >
            Send Message
          </button>
        </form>
